...
public:
    ...
    **/
    ...
    /**
    Warp the perspective of the current image
     @param from Points of the image defining the area to be warped
     @param to Points defining where to warp to inside a size Image
     @param size ...
    **/
    Image &perspectiveWarp(Point2f from[4], Point2f to[4], Size size);


    Image & extractQuad(Point2f * from, Size size);
    ...
    **/
    ...
    Plot the lines found using the Hough transform.
     ...
     @param channel Channel used. Default: 0
    **/
    Image &plotHoughLines(Image &plot, int channel = 0, Scalar color = Scalar(0,0,255));

    Image &plotHoughLines(int channel = 0, Scalar color = Scalar(0,0,255));
    ...
    **/
    ...
    uchar & at1(int i, int j);

    Vec3b & at3(int i, int j);
    Vec3b & at3(Point point);
